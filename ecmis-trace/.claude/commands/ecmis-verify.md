---
description: เทียบไฟล์ extract ของ Activity ที่ระบุกับ source code ใน input/code เติมข้อมูลชั้นหน้าจอ และระบุผลตรง/ไม่ตรงเล่ม
...
- ห้ามแก้ไฟล์ใน `input/code/` หรือ repo ที่ชี้ไป (อ่านอย่างเดียว)
- ต้องมี `work/extract/A{เลข 2 หลัก}_extract.md` ก่อน ถ้าไม่มีให้หยุดและบอกให้รัน `/ecmis-extract $ARGUMENTS`
...
7. รายงานผู้ใช้: จำนวน ตรงเล่ม / ไม่ตรงเล่ม / ไม่พบในโค้ด, รายการที่ไม่ตรงทั้งหมด, และรายการคำถามใหม่ แล้วหยุดรอผู้ใช้ก่อน `/ecmis-update`
