...
- `work/docs/` = เล่มที่แปลงเป็นข้อความพร้อมเลขหน้า (สร้างโดย `/ecmis-prepare`)
- `work/extract/` = ข้อมูลที่สกัดแยกราย Activity (สร้างโดย `/ecmis-extract`, `/ecmis-verify`)
...
1. **ห้ามเดา** ทุกช่องที่เติมต้องมีที่มาในคอลัมน์ `Source Ref / Page` (รูปแบบ: `ชื่อเล่ม น.xx` หรือ `path/ไฟล์:บรรทัด`) — ดูข้อ "ที่มาของแท็บที่ไม่มีคอลัมน์ Source Ref" ด้านล่าง
...
## ข้อสังเกตเกี่ยวกับโครงสร้าง template (ใช้ประกอบทุกคำสั่ง)
